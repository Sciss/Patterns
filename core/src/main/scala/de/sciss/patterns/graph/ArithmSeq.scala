...
final case class ArithmSeq[T1 <: Top, T2 <: Top, T <: Top](start: Pat[T1], step: Pat[T2] /* , length: Pat.Int = Int.MaxValue */)
  ...
  private final class StreamImpl[Tx](tx0: Tx)(implicit ctx: Context[Tx])
    ...
    protected def op(a: T#Out[Tx], b: T#Out[Tx]): T#Out[Tx] = num.plus(a, b)
  }
}
