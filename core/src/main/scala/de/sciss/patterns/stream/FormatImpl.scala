...
object FormatImpl extends StreamFactory {
  ...
  def expand[S <: Base[S]](pat: Format)(implicit ctx: Context[S], tx: S#Tx): patterns.Stream[S, String] = {
    import pat._
  ...
  def readIdentified[S <: Base[S], A](in: DataInput, access: S#Acc)(implicit tx: S#Tx): patterns.Stream[S, A] = {
    val sStream     = patterns.Stream.read[S, String](in, access)
    val argStreams  = Serializer.indexedSeq[S#Tx, S#Acc, patterns.Stream[S, Any]].read(in, access)
  ...
  private final class StreamImpl[S <: Base[S]](
                                                sStream   : patterns.Stream[S, String],
                                                argStreams: Vec[patterns.Stream[S, Any]]
  )
    ...
    protected def writeData(out: DataOutput): Unit = {
      sStream.write(out)
      Serializer.indexedSeq[S#Tx, S#Acc, patterns.Stream[S, Any]].write(argStreams, out)
    }
