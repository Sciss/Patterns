...
object IndexOfSliceImpl extends StreamFactory {
  ...
  def expand[S <: Base[S], A1, A2](pat: IndexOfSlice[A1, A2])(implicit ctx: Context[S], tx: S#Tx): patterns.Stream[S, Int] = {
    import pat._
    val id          = tx.newId()
  ...
  def readIdentified[S <: Base[S], A](in: DataInput, access: S#Acc)(implicit tx: S#Tx): patterns.Stream[S, A] = {
    ...
    val subStream   = patterns.Stream.read[S, Any](in, access)
    val fromStream  = patterns.Stream.read[S, Int](in, access)
  ...
  private final class StreamImpl[S <: Base[S], A1, A2](
                                                        ...
                                                        subStream : patterns.Stream[S, A2],
                                                        fromStream: patterns.Stream[S, Int],
                                                        fromValue : S#Var[Int],
