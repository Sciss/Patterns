...
object ItImpl extends StreamFactory {
  ...
  def expand[S <: Base[S], A](pat: It[A])(implicit ctx: Context[S], tx: S#Tx): patterns.Stream[S, A] = {
    import pat._
  ...
  def readIdentified[S <: Base[S], A](in: DataInput, access: S#Acc)(implicit tx: S#Tx): patterns.Stream[S, A] = {
    val refStream  = patterns.Stream.read[S, A](in, access)

    new StreamImpl[S, A](refStream = refStream)
  }


  private final class StreamImpl[S <: Base[S], A](refStream: patterns.Stream[S, A])
    extends Stream[S, A] {
