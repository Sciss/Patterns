...
object Tuple2_1Impl extends StreamFactory {
  ...
  def expand[S <: Base[S], A, A1](pat: Tuple2_1[A, A1])(implicit ctx: Context[S], tx: S#Tx): patterns.Stream[S, A] = {
    import pat._
  ...
  def readIdentified[S <: Base[S], A](in: DataInput, access: S#Acc)(implicit tx: S#Tx): patterns.Stream[S, A] = {
    val tupStream = patterns.Stream.read[S, (A, Any)](in, access)
    new StreamImpl[S, A, Any](tupStream = tupStream)
  }

  private final class StreamImpl[S <: Base[S], A, A1](tupStream: patterns.Stream[S, (A, A1)])
    extends Stream[S, A] {
