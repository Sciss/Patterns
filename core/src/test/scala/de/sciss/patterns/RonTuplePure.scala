...
//    println(s"NUM PATS    = ${Pat   .COUNT}")
  ...
  // collects the indices of every occurrence of elements of t in s
  def extract[A: ScalarEq](s: Pat[A], t: Pat[A]): Pat[Pat[Int]] =
    t.bubble.map { tj =>
  ...
  // computes and sorts all possible sub patterns of a pattern
  def computeDurs[A: ScalarEq](pattern: Pat[A], cantus: Pat[A], start: Pat[Int] = 0): Pat[Int] = {
  ...
  def makePart[A: ScalarEq](pattern: Pat[A], cantus: Pat[A], rest: A, start: Pat[Int] = 0,
