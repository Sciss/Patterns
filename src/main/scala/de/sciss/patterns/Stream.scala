...
/** A UGen during graph building process is a more
  ...
  */
sealed trait Stream extends Product {
  ...
  final def canEqual(x: Any): Boolean = x.isInstanceOf[Stream]
...
sealed trait StreamInLike extends PE {
  ...
  // ---- GE ----
  final private[patterns] def expand(implicit b: StreamGraph.Builder): StreamInLike = this
}
...
package graph {
  ...
  sealed trait Constant extends StreamIn {
